...
5) Save tokens to `~/.openclaw/whoop/token.json`:
...
- Copy the latest `access_token` + `refresh_token` from Postman into `~/.openclaw/whoop/token.json` and update `obtained_at`.
